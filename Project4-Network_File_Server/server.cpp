...
unordered_map<string, string> user_list;											//map username to corresponding password											
unordered_map<unsigned int, string> session_to_username;							//map session number to owner of this session
unordered_map<unsigned int, unsigned int> session_to_sequence;						//map session number to existing maximum sequence number of this session
unordered_map<int, pthread_rwlock_t*> lock_map;										//map block number to its lock
...
void request_handler(int msgsock)
{
	...
	if(strcmp (request_type, "FS_SESSION") == 0)
	{
		...
		//reconstructed string doesn't match original string
		if(trash != result_buf)
		{
			delete[] result_buf;
			close(msgsock);
		...
		// link the new session to this username and link the sequence number to this new session
	...
	else if(strcmp (request_type, "FS_READBLOCK") == 0)
	{
		...
		//reconstructed string doesn't match original string || block too lagre || pathname too long
		...
		//if this inode is not owned by the current user
		...
		//if the target inode is file and this data block index exists
	...
	else if(strcmp(request_type, "FS_CREATE") == 0)
	{
		...
		//reconstructed string doesn't match original string || pathname too long || create type not file or directory
	...
	else if(strcmp(request_type, "FS_DELETE") == 0)
	{
		...
		//reconstructed string doesn't match original string || pathname larger than maximum pathname
		...
		//if the owner of this block(being deleted) is not owned by current user
